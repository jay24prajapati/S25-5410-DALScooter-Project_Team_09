...
def lambda_handler(event, context):
    print(f"Processing login notifications: {json.dumps(event)}")
    ...
    for record in event.get('Records', []):
        try:
            ...
            notification_id = process_login_notification(actual_message, message_attributes)
            processed_count += 1
            
            print(f"Successfully processed login notification: {notification_id}")
    ...
    return {
        ...
        'body': json.dumps({
            ...
            'failed': failed_count,
            'message': 'Login notifications processed'
        })
    }

def process_login_notification(message, attributes):
    ...
    login_time = message.get('loginTime', timestamp)
    event_type = message.get('event', 'user_login')
    ...
    try:
        ...
        print(f"   Security alert would be sent here")
        
        # Update status to sent
        update_notification_status(notification_id, timestamp, 'sent')
        print(f"Login notification processed for user: {user_id}")
