  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`flex items-center px-6 py-3 rounded-xl font-semibold transition-all shadow-lg ${
                activeTab === tab.id && tab.id !== "analytics"
        ...
        <div className="mb-16">
          {activeTab === "overview" && <OverviewSection bookings={[]} tickets={tickets} bikes={bikes} />}
          ...
          {activeTab === "messages" && <ConversationList onOpenChat={(bookingId) => setSelectedBookingId(bookingId)} />}

          {selectedBookingId && <ChatModal bookingId={selectedBookingId} onClose={() => setSelectedBookingId(null)} />}
        </div>
      </div>
    </div>
  )
