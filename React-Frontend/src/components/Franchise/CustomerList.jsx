      ...
      <ul className="space-y-3">
        {customers.map(c => (
          <li key={c.customer_id} className="p-4 border rounded-lg bg-gray-50">
            ...
            <div className="text-sm text-gray-600">Total Bookings: {c.totalBookings} | Revenue: ${c.totalRevenue}</div>
            <div className="text-xs text-gray-400">Registered on: {new Date(c.registrationDate).toLocaleDateString()}</div>
          </li>
        ))}
      </ul>
    </div>
